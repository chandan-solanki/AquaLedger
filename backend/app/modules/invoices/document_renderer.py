"""ReportLab Platypus renderer for the Invoice PDF (Sprint 12 Session 2) -
the first concrete implementation of BaseDocumentRenderer
(app.core.document_engine.base_document). Takes an already-built
DocumentData (see app.modules.invoices.document_builder) and lays it out
with Platypus flowables (Table/Paragraph/Spacer/KeepTogether) so a
multi-page invoice paginates correctly - no raw canvas positioning of the
document body, and no financial calculation of any kind: every number
rendered here is read straight off DocumentData, formatted for display
only (`_money`/`_quantity` below never add/subtract/multiply anything).

Importing this module registers InvoiceDocumentRenderer into the shared
DocumentRegistry singleton for DocumentType.INVOICE - app.modules.invoices.router
does `import app.modules.invoices.document_renderer  # noqa: F401` once, at
module load time, mirroring app.core.report_export.exporters' own
registration-on-import pattern for PDF/Excel/CSV.

Page-numbering/footer machinery is shared with
app.modules.purchase.document_renderer (Session 3) via
...
_TENANT_NAME_STYLE = ParagraphStyle(
    "InvoiceTenantName", parent=_STYLES["Heading1"], fontSize=16, leading=20, alignment=TA_LEFT
)
_DOCUMENT_TITLE_STYLE = ParagraphStyle(
    "InvoiceDocumentTitle", parent=_STYLES["Normal"], fontSize=10, leading=14, alignment=TA_RIGHT
)
...
_BODY_STYLE = ParagraphStyle("InvoiceBody", parent=_STYLES["Normal"], fontSize=9, leading=12)
_BODY_RIGHT_STYLE = ParagraphStyle("InvoiceBodyRight", parent=_BODY_STYLE, alignment=TA_RIGHT)
_TABLE_HEADER_STYLE = ParagraphStyle(
    "InvoiceTableHeader", parent=_BODY_STYLE, fontName="Helvetica-Bold", textColor=colors.white
)
...
def _build_party_section(data: DocumentData) -> list[Flowable]:
    ...
    return [
        _build_card(
            [
                Paragraph("Bill To", _SECTION_HEADING_STYLE),
                Paragraph(body, _BODY_STYLE),
            ]
        )
    ]
...
def _build_sections(data: DocumentData) -> list[Flowable]:
    """Renders DocumentData.sections generically (e.g. the invoice
    builder's own "Additional Charges" section for a non-zero transport_
    charge/other_charge - ARCHITECTURE.md §13.4) - the renderer has no
    knowledge of what a section means, only how to lay out its title and
    optional line rows."""
...
class InvoiceDocumentRenderer(BaseDocumentRenderer):
    """Renders an invoice DocumentData to PDF bytes via ReportLab
    Platypus. Multi-page invoices paginate correctly because every
    element is a flowable - `Table(..., repeatRows=1)` repeats the line-
    item header row on every page the table itself splits across, and
    `KeepTogether` keeps the totals block from splitting mid-block."""
    ...
    def render(self, data: DocumentData) -> bytes:
        ...
        doc = SimpleDocTemplate(
            ...
            # Uncompressed content streams - an invoice is a small, few-page
            # document, so the size cost is negligible, and it keeps the
            # PDF's text greppable for debugging/auditing.
            pageCompression=0,
        )
...
registry.register(DocumentType.INVOICE, InvoiceDocumentRenderer)
