"""ReportLab Platypus renderer for the Purchase Bill PDF (Sprint 12
Session 3) - mirrors app.modules.invoices.document_renderer's own flow
exactly (header/party/line-items/totals/notes), adapted to the Purchase
Bill's own DTO shape (no fish name; the counterparty is a supplier
rather than a customer, so the party section is labeled "Supplier"
rather than "Bill To"). Every number rendered here is read straight off
DocumentData, formatted for display only - no financial calculation of
any kind.

Importing this module registers PurchaseBillDocumentRenderer into the
shared DocumentRegistry singleton for DocumentType.PURCHASE_BILL -
app.modules.purchase.router does
`import app.modules.purchase.document_renderer  # noqa: F401` once, at
...
app.modules.invoices.document_renderer via
...
_TENANT_NAME_STYLE = ParagraphStyle(
    "PurchaseBillTenantName", parent=_STYLES["Heading1"], fontSize=16, leading=20, alignment=TA_LEFT
)
_DOCUMENT_TITLE_STYLE = ParagraphStyle(
    "PurchaseBillDocumentTitle",
...
_BODY_STYLE = ParagraphStyle("PurchaseBillBody", parent=_STYLES["Normal"], fontSize=9, leading=12)
_BODY_RIGHT_STYLE = ParagraphStyle("PurchaseBillBodyRight", parent=_BODY_STYLE, alignment=TA_RIGHT)
_TABLE_HEADER_STYLE = ParagraphStyle(
    "PurchaseBillTableHeader", parent=_BODY_STYLE, fontName="Helvetica-Bold", textColor=colors.white
)
...
def _build_sections(data: DocumentData) -> list[Flowable]:
    """Renders DocumentData.sections generically (e.g. the purchase
    bill builder's own "Additional Charges" section for a non-zero
    transport_charge/other_charge) - the renderer has no knowledge of
    what a section means, only how to lay out its title and optional
    line rows."""
...
class PurchaseBillDocumentRenderer(BaseDocumentRenderer):
    """Renders a purchase bill DocumentData to PDF bytes via ReportLab
    Platypus. Multi-page bills paginate correctly because every element
    is a flowable - `Table(..., repeatRows=1)` repeats the line-item
    header row on every page the table itself splits across, and
    ...
    def render(self, data: DocumentData) -> bytes:
        ...
        doc = SimpleDocTemplate(
            ...
            # Uncompressed content streams - a purchase bill is a small,
...
registry.register(DocumentType.PURCHASE_BILL, PurchaseBillDocumentRenderer)
