...
class TestCalculatePurchaseBillTotals:
    def test_no_items_gives_zero_aggregates(self) -> None:
        totals = calculate_purchase_bill_totals(
            [],
            transport_charge=Decimal("0"),
            other_charge=Decimal("0"),
            round_off=Decimal("0"),
        )
        assert totals == PurchaseBillTotals(
    ...
    def test_no_items_but_nonzero_charges_still_totals_correctly(self) -> None:
        """A brand-new purchase bill (no items yet) with a transport_charge
        must still show it in total_amount/balance_amount."""
        totals = calculate_purchase_bill_totals(
            [],
    ...
    def test_total_amount_includes_round_off(self) -> None:
        totals = calculate_purchase_bill_totals(
            [_line(line_total="1000.00")],
            transport_charge=Decimal("0"),
            other_charge=Decimal("0"),
        ...
        assert totals.total_amount == Decimal("999.60")

    def test_paid_amount_is_always_zero(self) -> None:
        totals = calculate_purchase_bill_totals(
            [_line(line_total="1000.00")],
            transport_charge=Decimal("0"),
            other_charge=Decimal("0"),
    ...
    def test_balance_amount_always_equals_total_amount(self) -> None:
        """Unlike invoices (which subtract an already-integrated
        paid_amount), purchase bills have no supplier-payment integration
        yet (Session 5) - paid_amount is always 0, so balance_amount is
        always exactly total_amount."""
        totals = calculate_purchase_bill_totals(
            [_line(line_total="1000.00")],
            transport_charge=Decimal("50.00"),
            other_charge=Decimal("0"),
        ...
        assert totals.balance_amount == totals.total_amount == Decimal("1050.00")
    ...
    def test_negative_round_off_exceeding_subtotal_raises_negative_total_error(self) -> None:
        """Not reachable today (round_off is always 0 - no request field
        sets it yet), but the domain function itself must still refuse to
        produce a negative total_amount."""
        with pytest.raises(NegativeTotalError):
            calculate_purchase_bill_totals(
                [_line(line_total="100.00")],
                transport_charge=Decimal("0"),
                other_charge=Decimal("0"),
                round_off=Decimal("-200.00"),
            )
    ...
    def test_sum_is_rounded_half_up_to_two_decimal_places(self) -> None:
        ...
        decimal places, the bill-level aggregate must still come out
