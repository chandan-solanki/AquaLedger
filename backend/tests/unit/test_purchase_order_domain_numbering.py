...
class TestFiscalYearFor:
    ...
    def test_indian_gst_fiscal_year_boundaries(self, order_date: dt.date, expected: str) -> None:
        assert fiscal_year_for(order_date) == expected

    def test_century_rollover_stays_two_digits(self) -> None:
        assert fiscal_year_for(dt.date(2099, 4, 1)) == "2099-00"


class TestFormatPurchaseOrderNumber:
    def test_pads_sequence_to_five_digits(self) -> None:
        assert format_purchase_order_number("PO", "2026-27", 1) == "PO/2026-27/00001"

    def test_does_not_truncate_a_sequence_wider_than_the_padding(self) -> None:
        assert format_purchase_order_number("PO", "2026-27", 123456) == "PO/2026-27/123456"

    def test_uses_the_given_prefix_verbatim(self) -> None:
        assert format_purchase_order_number("XYZ", "2025-26", 42) == "XYZ/2025-26/00042"
