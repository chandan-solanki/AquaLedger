...
class TestCalculatePurchaseOrderTotals:
    def test_no_items_gives_zero_aggregates(self) -> None:
        ...
        assert totals == PurchaseOrderTotals(
    ...
    def test_no_items_but_nonzero_charges_still_totals_correctly(self) -> None:
        """A brand-new purchase order (no items yet) with a transport_charge
        must still show it in total_amount."""
        totals = calculate_purchase_order_totals(
            [],
    ...
    def test_sum_is_rounded_half_up_to_two_decimal_places(self) -> None:
        ...
        decimal places, the order-level aggregate must still come out
