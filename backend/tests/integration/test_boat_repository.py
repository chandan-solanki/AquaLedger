...
class TestGetById:
    async def test_finds_boat_in_own_tenant(
        ...
        boat = await _make_boat(db_session, tenant_id, name="Findable Boat")
    ...
    async def test_excludes_soft_deleted_rows(
        ...
        boat = await _make_boat(db_session, tenant_id, deleted_at=datetime.now(UTC))
...
class TestSearchFilters:
    async def test_filters_by_boat_type_case_insensitively(
        ...
        trawler = await _make_boat(db_session, tenant_id, boat_type="Trawler")
        await _make_boat(db_session, tenant_id, boat_type="Gillnetter")
    ...
    async def test_filters_by_is_active(
        ...
        inactive = await _make_boat(db_session, tenant_id, is_active=False)
    ...
    async def test_filters_by_insurance_expired_true(
        ...
        expired = await _make_boat(db_session, tenant_id, insurance_expiry=_PAST)
        await _make_boat(db_session, tenant_id, insurance_expiry=_FUTURE)
        await _make_boat(db_session, tenant_id, insurance_expiry=None)
    ...
    async def test_filters_by_insurance_expired_false_includes_unset(
        ...
        await _make_boat(db_session, tenant_id, insurance_expiry=_PAST)
        not_expired = await _make_boat(db_session, tenant_id, insurance_expiry=_FUTURE)
        unset = await _make_boat(db_session, tenant_id, insurance_expiry=None)
    ...
    async def test_filters_by_license_expired_true(
        ...
        expired = await _make_boat(db_session, tenant_id, license_expiry=_PAST)
        await _make_boat(db_session, tenant_id, license_expiry=_FUTURE)
    ...
    async def test_combines_filters(
        ...
        target = await _make_boat(db_session, tenant_id, boat_type="trawler", is_active=True)
        await _make_boat(db_session, tenant_id, boat_type="trawler", is_active=False)
        await _make_boat(db_session, tenant_id, boat_type="gillnetter", is_active=True)
    ...
    async def test_excludes_soft_deleted_from_results(
        ...
        await _make_boat(db_session, tenant_id, deleted_at=datetime.now(UTC))
...
class TestSearchQuery:
    ...
    async def test_matches_each_documented_field_case_insensitively(
        ...
        target = await _make_boat(db_session, tenant_id, **{field: value})
        await _make_boat(db_session, tenant_id)  # noise row that shouldn't match
    ...
    async def test_blank_query_returns_everything(
        ...
        await _make_boat(db_session, tenant_id)
        await _make_boat(db_session, tenant_id)
...
class TestSearchSorting:
    async def _seed_three(self, db_session: AsyncSession, tenant_id: uuid.UUID) -> None:
        await _make_boat(db_session, tenant_id, code="B-CODE", name="Bravo")
        await _make_boat(db_session, tenant_id, code="A-CODE", name="Alpha")
        await _make_boat(db_session, tenant_id, code="C-CODE", name="Charlie")
    ...
    async def test_id_tie_break_follows_the_primary_sort_direction(
        ...
        older_id_row = await _make_boat(db_session, tenant_id, created_at=tied_at)
        newer_id_row = await _make_boat(db_session, tenant_id, created_at=tied_at)
...
class TestSearchPagination:
    async def test_page_size_limits_rows_and_total_reflects_full_count(
        ...
        for i in range(5):
            await _make_boat(db_session, tenant_id, code=f"P-{i}", name=f"Page Boat {i}")
    ...
    async def test_pages_do_not_overlap_and_cover_all_rows(
        ...
        for i in range(5):
            await _make_boat(db_session, tenant_id, code=f"Q-{i}", name=f"Page Boat {i}")
...
class TestSearchTenantScoping:
    async def test_never_returns_rows_from_another_tenant(
        ...
        mine = await _make_boat(db_session, tenant_id, name="Mine")
        await _make_boat(db_session, other_tenant.id, name="Not Mine")
