...
/** Thin proxy for GET /invoices/{id}/document - the backend's response
 ...
 * NextResponse.json(), mirroring /api/reports/export's own binary
 * passthrough (Sprint 11 Session 5 Phase B / Sprint 12 Session 2). */
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const backendResponse = await authenticatedBackendBinaryRequest(`/invoices/${id}/document`);
    ...
    return new NextResponse(body, {
      ...
      headers: {
        ...
        // Every caller's URL is identical regardless of which invoice's PDF
        // they get back - an explicit signal so no browser/intermediate
        // cache reuses one tenant's invoice for another (mirrors
