...
/** Thin proxy for GET /purchase/{id}/document - the backend's response
 ...
 * binary passthrough (Sprint 12 Session 2/3). */
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const backendResponse = await authenticatedBackendBinaryRequest(`/purchase/${id}/document`);
