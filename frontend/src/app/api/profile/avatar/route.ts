...
/** Streams the avatar bytes for inline display - the backend deliberately
 * omits Content-Disposition: attachment for this endpoint (mirrors
 * /company-profile/logo), so this proxy doesn't fabricate one either. */
export async function GET(_request: NextRequest) {
  try {
    const backendResponse = await authenticatedBackendBinaryRequest("/profile/avatar");
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await authenticatedBackendFormRequest("/profile/avatar", formData);
...
export async function DELETE(_request: NextRequest) {
  try {
    await authenticatedBackendRequest("/profile/avatar", { method: "DELETE" });
