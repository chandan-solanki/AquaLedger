"use client";

import { format, parseISO } from "date-fns";
import { useRouter } from "next/navigation";
import { X } from "lucide-react";
import { useCallback, useMemo } from "react";
import type { DateRange } from "react-day-picker";

import {
  DataTable,
  DataTableEmpty,
  DataTableNoResults,
  DataTablePagination,
  useDataTable,
} from "@/components/data-table";
import { BooleanFilter, DateRangeFilter, SearchBar, StatusFilter } from "@/components/filters";
import { PageContainer } from "@/components/layout/page-container";
import { ExportMenu } from "@/components/reports";
import { ReportPageTemplate } from "@/components/templates/report-page-template";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Button } from "@/components/ui/button";
import { Forbidden } from "@/components/feedback/error-states";
import { getOutstandingReportColumns } from "@/features/reports/components/outstanding-report-columns";
import { OutstandingReportSummaryCards } from "@/features/reports/components/outstanding-report-summary-cards";
import { RISK_LEVEL_OPTIONS } from "@/features/reports/constants/risk-level";
import { useOutstandingReport } from "@/features/reports/hooks/use-outstanding-report";
import { useOutstandingReportFilters } from "@/features/reports/hooks/use-outstanding-report-filters";
import {
  toOutstandingReportParams,
  type OutstandingReportFilters,
} from "@/features/reports/schemas/outstanding-report-filters";
import type { OutstandingReportRow } from "@/features/reports/types/outstanding-report";
import { triggerReportDownload } from "@/features/reports/utils/trigger-report-download";
import { useExternalValueKey } from "@/hooks/use-external-value-key";
import { normalizeApiError } from "@/utils/api-error";

function toDateRange(filters: OutstandingReportFilters): DateRange | undefined {
  if (!filters.fromDate && !filters.toDate) return undefined;
  return {
    from: filters.fromDate ? parseISO(filters.fromDate) : undefined,
    to: filters.toDate ? parseISO(filters.toDate) : undefined,
  };
}

const ISO_DATE_FORMAT = "yyyy-MM-dd";

/**
 * The Outstanding Report page (TASKS.md Sprint 11 Session 3 Phase B) -
 * Tabs (Customer/Supplier) -> Filter Bar -> Summary Cards -> Report Table,
 * driven entirely by the backend's single GET /reports/outstanding
 * response. One row is one business entity, never one transaction.
 * Clicking a row drills into that entity's Customer/Supplier Ledger
 * (reusing the existing routes from Sessions 1-2) - no extra permission
 * gate beyond `reports:view`, since both pages require the same
 * permission this page itself already required to load.
 */
export function OutstandingReportPage() {
  const router = useRouter();
  const [filters, setFilters] = useOutstandingReportFilters();
  const [searchKey, reportSearch] = useExternalValueKey(filters.search);
  const query = useOutstandingReport(filters);

  const data = query.data;
  const apiError = query.isError ? normalizeApiError(query.error) : null;

  const applyFilterChange = useCallback(
    (patch: Partial<Omit<OutstandingReportFilters, "page">>) => {
      setFilters({ ...patch, page: 1 });
    },
    [setFilters]
  );
  const goToPage = useCallback((page: number) => setFilters({ page }), [setFilters]);
  const setPageSize = useCallback(
    (pageSize: number) => setFilters({ pageSize, page: 1 }),
    [setFilters]
  );
  const resetFilters = useCallback(() => setFilters(null), [setFilters]);

  const dateRange = useMemo(() => toDateRange(filters), [filters]);
  const hasActiveFilters = Boolean(
    filters.search.trim() ||
      filters.outstandingOnly ||
      filters.overdueOnly ||
      filters.riskLevel ||
      filters.fromDate ||
      filters.toDate
  );

  const rows = useMemo(() => data?.rows ?? [], [data]);
  const columns = useMemo(
    () => getOutstandingReportColumns(filters.entityType),
    [filters.entityType]
  );
  const table = useDataTable({
    data: rows,
    columns,
    pageCount: Math.max(1, Math.ceil((data?.pagination.totalRecords ?? 0) / filters.pageSize)),
  });

  if (apiError?.category === "forbidden") {
    return (
      <PageContainer>
        <Forbidden description="You don't have permission to view accounting reports. Contact an administrator if you believe this is a mistake." />
      </PageContainer>
    );
  }

  const tabs = (
    <Tabs
      value={filters.entityType}
      onValueChange={(value) => applyFilterChange({ entityType: value as "customer" | "supplier" })}
    >
      <TabsList>
        <TabsTrigger value="customer">Customer Outstanding</TabsTrigger>
        <TabsTrigger value="supplier">Supplier Outstanding</TabsTrigger>
      </TabsList>
    </Tabs>
  );

  const filterBar = (
    <div className="flex flex-col gap-3">
      {tabs}
      <div className="flex flex-wrap items-end gap-3">
        <SearchBar
          key={searchKey}
          defaultValue={filters.search}
          onSearch={(value) => {
            reportSearch(value);
            applyFilterChange({ search: value });
          }}
          placeholder="Search by name or code…"
          isLoading={query.isFetching}
          aria-label="Search entities"
          className="min-w-56 flex-1"
        />
        <DateRangeFilter
          label="Date Range"
          value={dateRange}
          onChange={(range) =>
            applyFilterChange({
              fromDate: range?.from ? format(range.from, ISO_DATE_FORMAT) : null,
              toDate: range?.to ? format(range.to, ISO_DATE_FORMAT) : null,
            })
          }
        />
        <StatusFilter
          label="Risk Level"
          options={RISK_LEVEL_OPTIONS}
          value={filters.riskLevel ?? undefined}
          onChange={(value) => applyFilterChange({ riskLevel: value ?? null })}
        />
        <BooleanFilter
          label="Outstanding Only"
          checked={filters.outstandingOnly}
          onChange={(checked) => applyFilterChange({ outstandingOnly: checked })}
        />
        <BooleanFilter
          label="Overdue Only"
          checked={filters.overdueOnly}
          onChange={(checked) => applyFilterChange({ overdueOnly: checked })}
        />
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={resetFilters}>
            <X aria-hidden />
            Reset
          </Button>
        )}
      </div>
    </div>
  );

  return (
    <ReportPageTemplate
      title="Outstanding Report"
      description="Every customer and supplier with invoice/purchase-bill history, with a dynamic risk indicator."
      exportMenu={
        <ExportMenu
          onExport={(format) =>
            triggerReportDownload("outstanding_report", format, toOutstandingReportParams(filters))
          }
        />
      }
      filters={filterBar}
      summary={data ? <OutstandingReportSummaryCards summary={data.summary} /> : undefined}
      isLoading={query.isLoading}
      error={
        apiError
          ? {
              title: "Failed to load the outstanding report",
              description: apiError.message,
              onRetry: () => query.refetch(),
            }
          : null
      }
    >
      <DataTable
        table={table}
        isLoading={query.isFetching}
        loadingRowCount={Math.min(filters.pageSize, 10)}
        isEmpty={!query.isLoading && !apiError && rows.length === 0 && !hasActiveFilters}
        emptyState={
          <DataTableEmpty
            title={
              filters.entityType === "customer" ? "No customers yet" : "No suppliers yet"
            }
            description="Entities with invoice/purchase-bill history will appear here."
          />
        }
        isNoResults={!query.isLoading && !apiError && rows.length === 0 && hasActiveFilters}
        noResultsState={<DataTableNoResults onClearFilters={resetFilters} />}
        pagination={
          <DataTablePagination
            pageIndex={filters.page - 1}
            pageSize={filters.pageSize}
            totalCount={data?.pagination.totalRecords ?? 0}
            onPageChange={(pageIndex) => goToPage(pageIndex + 1)}
            onPageSizeChange={setPageSize}
          />
        }
        onRowClick={(row: OutstandingReportRow) =>
          router.push(
            filters.entityType === "customer"
              ? `/reports/customer-ledger?customerId=${row.entityId}`
              : `/reports/supplier-ledger?supplierId=${row.entityId}`
          )
        }
        stickyHeader
        aria-label="Outstanding report"
      />
    </ReportPageTemplate>
  );
}
