...
/**
 * All companies for this tenant, for the Payment list's Company filter and
 * for resolving `company_id` to a display name in the Customer column -
 * `PaymentResponse` carries only `company_id` (app/modules/payments/schemas.py),
 * never a nested company name, so the payment list resolves it client-side
 ...
 * mirroring `useInvoices`'s own `use-company-options.ts` exactly.
 */
