...
/**
 * All suppliers for this tenant, for the Purchase Bill list's Supplier
 * filter and for resolving `supplier_id` to a display name in the Supplier
 * column - `PurchaseBillResponse` carries only `supplier_id`
 * (app/modules/purchase/schemas.py), never a nested supplier name, so the
 * purchase bill list resolves it client-side through the Suppliers
 * feature's own public surface (`@/features/suppliers`) rather than the
 * backend joining it server-side (modules never reach into another
 * feature's internals directly - `07_FRONTEND_ARCHITECTURE.md` §5),
 * mirroring `useCompanyOptions` (`invoices` feature) exactly.
 */
